	...
	function create_each_block(component, ctx) {
		...
		return {
			c() {
				...
				div.className = div_class_value = "listItem " + (ctx.item.active ? 'active' : '') + " svelte-ocijb9";
			},
			...
			p(changed, ctx) {
				...
				if ((changed.items) && div_class_value !== (div_class_value = "listItem " + (ctx.item.active ? 'active' : '') + " svelte-ocijb9")) {
					div.className = div_class_value;
				}
			},
